...
class ImageController {
     ...
     * 获取图片信息
     */
    static async getImageInfo(request, reply) {
        try {
            const { imageId } = request.params;
            const image = await imageService_1.imageService.getImageInfo(imageId);
            if (!image) {
     ...
     * 图片代理访问
     */
    static async proxyImage(request, reply) {
        try {
            const { imageId } = request.params;
            const { width, height, quality, format } = request.query;
            // 如果有处理参数，返回处理后的图片
            if (width || height || quality || format) {
                const processedBuffer = await imageService_1.imageService.processImageOnDemand(imageId, {
            ...
            // 否则获取最佳匹配的URL并重定向
            const imageUrl = await imageService_1.imageService.getImageProxy(imageId, {
                width: width ? parseInt(String(width)) : undefined,
                height: height ? parseInt(String(height)) : undefined
            });
            if (!imageUrl) {
     ...
     * 上传单个图片
     */
    static async uploadSingle(request, reply) {
        try {
            ...
            const image = await imageService_1.imageService.uploadImage(buffer, filename, productId, type);
            return {
                success: true,
                data: {
                    imageId: image.imageId,
            ...
            return reply.status(500).send({
                success: false,
                error: { message: `图片上传失败: ${error.message}` }
            });
        }
    }
    /**
     * 批量上传产品图片
     */
    static async uploadBatch(request, reply) {
        try {
            ...
            const results = await imageService_1.imageService.uploadProductImages(productId, files);
            return {
                success: true,
                data: {
                    productId,
            ...
            return reply.status(500).send({
                success: false,
                error: { message: `批量图片上传失败: ${error.message}` }
            });
        }
    }
    /**
     * 获取产品的所有图片
     */
    static async getProductImages(request, reply) {
        try {
            const { productId } = request.params;
            const images = await imageService_1.imageService.getProductImages(productId);
            return {
                success: true,
                data: {
                    productId,
     ...
     * 删除图片
     */
    static async deleteImage(request, reply) {
        try {
            const { imageId } = request.params;
            const success = await imageService_1.imageService.deleteImage(imageId);
            if (!success) {
                return reply.status(404).send({
                    success: false,
     ...
     * 图片服务健康检查
     */
    static async healthCheck(request, reply) {
        try {
            const healthStatus = await imageService_1.imageService.healthCheck();
    ...
    static async getStats(request, reply) {
        try {
            const stats = await Image_1.Image.aggregate([
                { $match: { isActive: true } },
                {
                    $group: {
                        _id: null,
            ...
            const typeStats = await Image_1.Image.aggregate([
                { $match: { isActive: true } },
                {
                    $group: {
                        _id: '$type',
     ...
     * 验证上传参数
     */
    static validateUploadParams(data, type, buffer) {
        // 验证图片类型
        const validTypes = ['front', 'back', 'label', 'package', 'gift'];
        if (!validTypes.includes(type)) {
