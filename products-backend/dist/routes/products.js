...
async function productRoutes(fastify) {
    ...
    fastify.get('/products', async (request, reply) => {
        try {
            const { page = 1, limit = 20, category, platform, search, priceMin, priceMax, province, sortBy = 'time', sortOrder = 'desc', status = 'active' } = request.query;
            // 构建查询条件
            ...
            if (province)
                query['origin.province'] = province;
            if (search) {
            ...
            if (priceMin !== undefined || priceMax !== undefined) {
                ...
                if (priceMin !== undefined)
                    query['price.normal'].$gte = priceMin;
                if (priceMax !== undefined)
                    query['price.normal'].$lte = priceMax;
            }
            // 构建排序条件
            let sortCondition = {};
            switch (sortBy) {
    ...
    fastify.get('/products/:id', async (request, reply) => {
        try {
            ...
            // 查询相关产品（同分类的其他产品）
            const relatedProducts = await models_1.Product.find({
    ...
    // 批量获取产品
    fastify.post('/products/batch', async (request, reply) => {
        try {
            ...
            let productQuery = models_1.Product.find(query);
            // 如果指定了字段，只选择这些字段
            if (fields && Array.isArray(fields) && fields.length > 0) {
                productQuery = productQuery.select(fields.join(' '));
            }
